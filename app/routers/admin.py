...
def delete_prerequisite(
    ...
    """
    Delete a prerequisite. Rewires dependencies, cleans up columns,
    re-normalises sort_order, and auto-syncs prereq_tails.
    """
    row = db.query(MilestoneDefinition).filter(MilestoneDefinition.id == prerequisite_id).first()
    if not row:
        raise HTTPException(status_code=404, detail=f"Prerequisite with id {prerequisite_id} not found")
    ...
    deleted_key = row.key
    deleted_depends_on = row.depends_on
    ...
    for r in all_rows:
        ...
        if isinstance(dep, list):
            if deleted_key in dep:
                parent = _parse_depends_on(deleted_depends_on)
                if parent is None:
                    new_dep = [d for d in dep if d != deleted_key]
                elif isinstance(parent, list):
                    new_dep = [d for d in dep if d != deleted_key] + parent
                else:
                    new_dep = [parent if d == deleted_key else d for d in dep]
                seen = set()
                deduped = []
                for d in new_dep:
                    if d not in seen:
                        seen.add(d)
                        deduped.append(d)
                r.depends_on = json.dumps(deduped) if len(deduped) > 1 else (deduped[0] if deduped else None)
        else:
            if dep == deleted_key:
                r.depends_on = deleted_depends_on

    # --- Delete milestone columns ---
    db.query(MilestoneColumn).filter(MilestoneColumn.milestone_key == deleted_key).delete()

    # --- Delete prereq tail entry if exists ---
    db.query(PrereqTail).filter(PrereqTail.milestone_key == deleted_key).delete()

    # --- Delete the milestone definition ---
    db.delete(row)
    db.flush()
    ...
    # --- Recompute dependencies and sync tails ---
    _recompute_and_persist_dependencies(db)
    _sync_prereq_tails(db)

    db.commit()

    return {
        "detail": f"Prerequisite '{deleted_key}' deleted. Dependencies, tails, and sort order updated.",
